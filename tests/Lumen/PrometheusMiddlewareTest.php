...
use Anik\Laravel\Prometheus\Test\Laravel\InteractsWithStorage;
use Closure;
...
class PrometheusMiddlewareTest extends TestCase
{
    ...
    protected function getRouteHandler(): Closure
    {
        return function (Request $request): JsonResponse {
            ...
            return new JsonResponse(['success' => $code >= 200 && $code < 400], $code);
        };
    }
    ...
    {
        $middlewares = Closure::bind(fn() => $this->middleware, $this->app)->call($this->app);
        $this->assertContains(PrometheusMiddleware::class, $middlewares);
    }

    /** @pre-service-register disableRequestMetrics */
    public function testRequestMetricsCanBeDisabledUsingConfig()
    {
        $middlewares = Closure::bind(fn() => $this->middleware, $this->app)->call($this->app);
        $this->assertNotContains(PrometheusMiddleware::class, $middlewares);
    }
    ...
    public function testMetricsWillBeStoredIfLaravelStartConstantIsSet()
    {
    ...
    public function testRequestMetricsWillBeSkippedIfHttpMethodsMatchDataInIgnoreArray(array $rules)
    {
        ...
        foreach ($rules['routes'] ?? [] as $route) {
            ...
            $this->{$verb}($route[1])->assertResponseOk();
        }
    }
